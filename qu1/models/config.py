...
class RWKVConfig(eqx.Module):
    ...
    def __post_init__(self):
        ...
        if self.d_w is None:
            self = replace(self, d_w=d_w)
        if self.d_a is None:
            self = replace(self, d_a=d_a)
        if self.d_g is None:
            self = replace(self, d_g=d_g)
        if self.d_v is None:
            self = replace(self, d_v=d_v)
        if self.d_ff is None:
            self = replace(self, d_ff=self.d_model * 4)
        if self.head_size is None:
            assert self.d_model % self.head_size == 0, "d_model must be divisible by head_size"
            self = replace(self, head_size=self.d_model // self.n_head)
        elif self.n_head is None:
            assert self.d_model % self.n_head == 0, "d_model must be divisible by n_head"
            self = replace(self, n_head=self.d_model // self.head_size)
        else:
